...
const OrderForm = ({ estimate, onClose }) => {
  ...
  const [orderSuccessDialogStatus, setOrderSuccessDialogStatus] =
    useState(false);
  ...
  return (
    <CSSTransition
      in={orderSuccessDialogStatus}
      timeout={100}
      classNames="estimate"
    >
      {orderSuccessDialogStatus ? (
        <OrderDialog
          title="Order Request Successfull"
        ...
        <Box sx={{ bg: "secondary" }}>
          ...
          <Box sx={{ strong: { fontWeight: "bold" }, p: 3, color: "primary" }}>
            ...
            <Box className="order-form-features">
              <h4 sx={{ mb: 2 }}>Included Features</h4>
              <Box>
                <Box sx={{ mb: 2 }}>
                  <strong>Contact Form: </strong>
                  <span>
                    {estimate.hasContactForm ? (
            ...
            <p className="order-form-total">
              Estimated Total: <span>${estimate.totalCost?.toFixed(2)}</span>
            </p>
            <Box sx={{ mt: 4 }}>
              <Button
                variant="primary"
              ...
              <Text sx={{ fontSize: 1 }}>
                This auto generated estimate is based on the commonly requested
                features. Additional features beyond those mentioned may result
